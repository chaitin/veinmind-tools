...
const (
	...
	PageHeaderSize          = 0x58
	ListSize                = 0x26
	PageDataSize            = 0x4000 - 0x38 - 0x58 - 0x26 + 0x8
	PageTailSize            = 0x8
)
...
func FindUserPage(f io.Reader) (page Page, err error) {
	...
	for {
		...
		// 认为 EmptyPasswordPlaceholder 所在的页就是 user 表的页
		// TODO: 如果修改了 MySQL 保留账户的密码，这一方法就失效了
		if IsUserPage(buf) {
...
func ParseUserPage(pagedata PageData) (infos []MysqlInfo, err error) {
	...
	InfimumPos := pagedata.Infimum_record.Recordheader.Next_record
	// 最小记录起始位置, 0xC - 0x8
	var startPos int16 = 0x5
	var endPos int16 = 0x12
	next_record := startPos + InfimumPos
	// 根据链表指针获取每行的数据
	for i := next_record; i != endPos; {
