...
func scan(c *cmd.Command, image api.Image) (err error) {
	...
	for _, app := range appType {
		ModuleResult, err := utils.StartModule(config, image, app)
		if err != nil {
			log.Warn(err)
			continue
		}
...
func init() {
	...
	scanCmd.Flags().StringSliceVarP(&appType, "apptype", "a", []string{"mysql", "tomcat", "redis", "ssh"}, "find weakpass in these app e.g. ssh")
}
